...
export async function dashboardAccess(request?: Request, admin = false) {
  if (!elasticVulnEnabled()) throw new DashboardError("Not found", 404);
  const session = await getServerSession(authOptions);
  const user = session?.user as { email?: string; name?: string; role?: string; orgMember?: boolean } | undefined;
  if (!user || user.orgMember === false) throw new DashboardError("Unauthorized", 401);
  const canManage = user.role === "ADMIN";
  if (admin && !canManage) throw new DashboardError("Only organization admins can manage Elastic queries.", 403);
  if (request && admin) {
  ...
  return { canManage, actor: user.email || user.name || "organization-admin" };
}
