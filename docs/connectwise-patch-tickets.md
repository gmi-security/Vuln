...
The API member needs access to read companies, service boards and teams; read/create service tickets; and read/upload ticket documents. Configure the member's security role in ConnectWise. Priority and initial status are assigned by ConnectWise defaults, so the dashboard no longer fetches or validates those options.

Connection testing, board lists, saved board selections, and pre-creation board validation use `GET /service/info/boards` and `GET /service/info/boards/{id}` (ticket inquiry access). They do not use the board setup endpoint. Team options use the board teams endpoint. A successful connection test verifies board reads only; it does not prove ticket creation or document-upload permission, and a failed board read does not prove ticket creation is forbidden. No test ticket is sent.

Choose a board and optional team from lists fetched from ConnectWise. No board names or IDs are hardcoded. Lists support pagination, company search, and loading saved selections outside the first page. Changing a board clears its team. Inactive selections cannot be used. Customer company is explicitly selected during each ticket review; it is distinct from the login company ID. Ticket creation omits both `status` and `priority` so ConnectWise applies its configured defaults; legacy saved selections are ignored. Existing tickets still show their returned status in history. If ConnectWise requires an explicit value because its defaults are not configured, its creation error is shown without an automatic retry.
...
Browser checks cover member access, secure key fields, real-name selection, board/team reset, omitted priority/status controls and lookups, defaults, ticket review, CSV retry, shared history, saved report resume, mobile layout, and existing any-CVE actions. A read-only production diagnostic on September 26, 2026 confirmed board/company lists and one board's status/team lists succeeded; `/service/priorities` returned HTTP 403. No live ticket was sent by verification; creation and attachment recovery are tested against mocked ConnectWise responses.
