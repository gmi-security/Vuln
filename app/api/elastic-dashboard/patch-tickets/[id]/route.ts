...
export async function GET(request: Request, context: Context) {
  try { await dashboardAccess(); return dashboardJson(await readPatchTicket((await context.params).id, new URL(request.url).searchParams.get("packet") === "1")); } catch (error) { return dashboardFailure(error); }
}
