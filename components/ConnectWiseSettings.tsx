...
export default function ConnectWiseSettings() {
  ...
  const [form, setForm] = useState({ endpoint: "", companyId: "", clientId: "", publicKey: "", privateKey: "" });
  ...
  useEffect(() => {
    ...
    dashboardRequest<CWSettings>("connectwise").then(data => { if (live) {
      setSettings(data); setDefaults(data.defaults); setForm(old => ({ ...old, endpoint: data.endpoint ?? "", companyId: data.companyId ?? "", clientId: data.clientId ?? "" }));
  ...
  async function save(path: string, value: unknown) {
    ...
    try {
      ...
      setSettings(data); setDefaults(data.defaults); setForm(old => ({ ...old, publicKey: "", privateKey: "" }));
  ...
  return <div className="space-y-6">
    <form className="space-y-4" onSubmit={e => { e.preventDefault(); void save("connectwise", form); }}>
      <fieldset disabled={Boolean(busy)} className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="block text-sm text-zinc-200">Login company ID<input required className={`${inputClass} mt-2`} value={form.companyId} onChange={e => setForm({ ...form, companyId: e.target.value })} autoComplete="off" spellCheck={false} /></label>
        ...
        <p className={`${styles.resultNote} sm:col-span-2`}>Keys are encrypted on the server and never returned to the browser. Use an API member with permission to read companies, boards, statuses, teams and priorities, create service tickets, and read/upload ticket documents. Your login company ID identifies your ConnectWise account; the ticket's customer is chosen separately.</p>
